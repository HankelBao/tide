...
impl TextEditing for TextBuffer {
    fn insert(&mut self, ch: char) {
        match ch {
            '\n' => {
                ...
                self.lines.insert(self.line_num as usize + 1, Box::new(TextLine::from(left_content)));
                self.line_num += 1;
                self.line_offset = 0;
            },
            '\t' => {
                ...
                for _ in self.line_offset..target_index {
                    self.lines[self.line_num as usize].insert(self.line_offset as usize, ' ');
                    self.line_offset += 1;
                }
            },
            _ => {
    ...
    fn backspace(&mut self) {
        ...
        if self.line_offset > 0 {
            if self.lines[self.line_num as usize].char_at(self.line_offset as usize-1) != ' ' {
            ...
            for i in (target_index..self.line_offset).rev() {
                if self.lines[self.line_num as usize].char_at(i as usize) != ' ' {
                    return
                }
                self.lines[self.line_num as usize].remove(self.line_offset as usize - 1);
                self.line_offset -= 1;
            ...
            let left_content = self.lines[self.line_num as usize].content();
    ...
    fn del_to_head(&mut self) {
        let line = self.lines[self.line_num as usize].content();
        let (_, right) = line.split_at(self.line_offset as usize);
        self.lines[self.line_num as usize] = Box::new(TextLine::from(right.to_string()));
        self.line_offset = 0;
    }
    ...
    fn del_to_end(&mut self) {
        ...
        self.lines[self.line_num as usize] = Box::new(TextLine::from(left.to_string()));
        self.line_offset = self.lines[self.line_num as usize].len() as u32;
    }
}